<div id="question_{{$id}}" class="w-full xl:w-[80%] h-auto flex flex-col py-4 sm:p-4">
    <div class="w-full h-auto flex flex-row justify-between items-center py-4">
        ...
        <div id="toggleButton_{{$id}}" class="h-12 w-12 rounded-full flex items-center justify-center bg-gray-light p-3 hover:bg-secondary-dark hover:cursor-pointer duration-300 active:scale-95 group">
            <div id="iconOpen_{{$id}}" class="w-full h-full">
                <x-heroicon-o-plus class="h-full w-full group-hover:text-white" />
            </div>
            <div id="iconClose_{{$id}}" class="hidden w-full h-full">
