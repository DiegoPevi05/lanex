    ...
    <div class="bg-white h-screen w-full">
        <div class="h-full w-full flex justify-center items-center">
            ...
            <div class="h-auto w-full sm:w-[400px] flex justify-center items-center flex-col shadow-xl border-2 border-gray-light rounded-xl p-6 gap-y-4">
                ...
                <p class="font-bold text-sm text-secondary-dark">{{__('messages.dashboard.auth.reset_password')}}</p>
                <form action="{{ route('password.update') }}" method="POST" class="w-full h-full flex flex-col gap-y-2">
                    @csrf
                    ...
                    <button type="submit" class="mt-4 mx-auto px-4 py-2 bg-primary text-white duration-300 hover:bg-primary-dark rounded-md active:scale-95 capitalize">{{__('messages.dashboard.auth.reset_password')}}</button>
