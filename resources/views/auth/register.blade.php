    ...
    <div class="bg-white h-screen w-full">
        <div class="h-full w-full flex justify-center items-center">
            <div class="h-auto w-full sm:w-[400px] flex justify-center items-center flex-col shadow-xl border-2 border-gray-light rounded-xl p-6 gap-y-4">
                ...
                <p class="font-bold text-sm text-secondary-dark">{{__('messages.dashboard.auth.register')}}</p>
                <form action="{{ route('register') }}" method="POST" class="w-full h-full flex flex-col items-start justify-center gap-y-2">
                    ...
                    <label for="keyword" class="block text-sm font-bold text-secondary-dark capitalize">{{ __("messages.dashboard.auth.keyword") }}</label>
                    <input type="text" id="keyword" name="keyword" class="mt-2 text-sm block w-full p-2 border-b-2 border-b-secondary-dark bg-white focus:border-b-primary focus:outline-none text-body" value="{{ old('keyword', '') }}" placeholder="{{ __("messages.dashboard.auth.keyword") }}">
